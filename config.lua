-- LC Housing
-- Created by LC
-- Configure the resource here; installation examples belong in README.md and install/.
...
Config.Debug = true
Config.DebugDoorScanFile = 'door_scan_debug.lua'

-- Debug drawing shows property zones, entrances, and garages in-world.
-- Leave enabled while building/testing houses, then disable if you want a cleaner live server.
Config.DebugDraw = {
    enabled = false, -- master toggle for all debug markers/text
    drawDistance = 160.0, -- how far away property debug zones are visible
    polygonZOffset = 0.08, -- raises zone lines slightly above the ground
    polygonGroundOffset = 0.05, -- lifts the bottom of the 3D property volume just above the ground
    polygonHeight = 18.0, -- height of the debug property box, normally reaching above the roof
    polygonFillAlpha = 32, -- transparency for the roof of the debug property box
    polygonWallAlpha = 22, -- transparency for the vertical walls of the debug property box
    polygonLineAlpha = 230, -- visibility for polyzone border lines
    garageDrawDistance = 160.0 -- how far garage debug markers are visible
}

-- Framework detection. Use auto unless you specifically need to force qb or qbox.
Config.Framework = 'auto' -- auto, qb, qbox
Config.FrameworkResources = {
    qb = 'qb-core', -- resource name for QBCore
    qbox = 'qbx_core' -- resource name for qbox/qbx_core
}

-- Commands.
-- /housing and /createhouse open the real estate/admin panel.
-- /myhouse opens owner management near an owned house or while inside one.
Config.Command = 'housing' -- opens the staff/real estate panel
Config.CreateCommand = 'createhouse' -- shortcut for the same admin panel
Config.OwnerCommand = 'myhouse' -- opens the owned-house menu for civilians
Config.OwnerCommandDistance = 35.0 -- distance checked when finding a nearby owned house
Config.OwnerCommandInteriorDistance = 25.0 -- recovery radius around a shell interior exit after this resource restarts

-- Real estate job access.
-- RequireOnDuty = true means agents must be clocked in to use the panel.
Config.JobName = 'realestate' -- job name allowed to use the management panel
Config.RequireOnDuty = false -- set true to require agents to be clocked in

-- Buying settings for civilian listings.
-- account can be bank or cash. allowCashFallback lets cash cover bank purchases if bank funds are short.
Config.Purchase = {
    enabled = true, -- allow civilians to buy listed houses
    account = 'bank', -- primary payment account: bank or cash
    allowCashFallback = true -- if bank is short, allow cash to cover the purchase
}

-- Minimum realestate job grade needed for each action.
-- Grade numbers match the job grades at the bottom of this file.
Config.Permissions = {
    view = 0, -- minimum grade to open/view the panel
    create = 0, -- minimum grade to create new properties
    edit = 1, -- minimum grade to edit existing properties
    delete = 3 -- minimum grade to delete properties
}

-- LC Housing always stores its authoritative property records in lc_houses.
-- Enable mirroring only when maintaining a legacy qb-houses houselocations table.
Config.MirrorQbHouses = false -- true also writes compatible rows to qb-houses/houselocations

-- Garage integration.
-- Uses qb-garages house garage events when enabled and the garage resource is running.
Config.Garage = {
    enabled = true, -- allow houses to have garage points
    resource = 'qb-garages', -- garage resource to sync house garages with
    promptDistance = 5.0 -- distance for garage interaction prompts
}

-- Exterior interaction settings.
-- hybrid (default) offers both E prompts and third-eye actions when qb-target/ox_target is running.
-- prompt = E only, target = third-eye only, auto = prompt plus optional target when enabled.
Config.Interaction = {
    mode = 'target', -- prompt, target, auto, hybrid
    targetResources = { 'qb-target', 'ox_target' },
    target = {
        enabled = true, -- used by auto/hybrid; target mode always enables third eye
        resource = 'auto', -- auto, qb-target, ox_target
        showManage = true, -- add a separate Manage Property third-eye action for owners
        drawSprite = false, -- target-only mode: do not draw target arrows/sprites in the world
        icon = 'fas fa-house', -- icon shown by target resources
        length = 1.2, -- target zone length
        width = 1.2, -- target zone width
        height = 2.0 -- target zone height
    },
    marker = {
        enabled = true, -- draw world markers for properties when using prompts
        type = 2, -- GTA marker type
        zOffset = 0.02, -- raises the marker above the saved point
        scale = { x = 0.28, y = 0.28, z = 0.28 }, -- marker size
        color = { r = 126, g = 182, b = 255, a = 150 } -- marker color/alpha
    },
    promptDistance = 2.0, -- distance to show/use E prompts
    drawDistance = 35.0 -- distance to draw exterior markers
}

-- Door locks for MLO/IPL exterior doors.
-- Add one entry per house ID. Use the door object's model name/hash and the exact door coordinates.
-- Example:
-- Config.DoorLocks.houses['little_seoul_#1'] = {
--     { model = 'v_ilev_fh_frontdoor', coords = { x = -14.87, y = -1441.18, z = 31.10 }, radius = 1.5, heading = 0.0 }
-- }
Config.DoorLocks = {
    enabled = true, -- master toggle for LC door lock handling
    applyDistance = 80.0, -- distance where door freeze/lock state is applied
    defaultRadius = 1.5, -- object search radius around saved door coords
    scanRadius = 120.0, -- admin bulk scan radius around the property zone
    addRadius = 3.0, -- owner/admin manual add search radius
    manualAddRadius = 2.0, -- tight fallback radius when facing/aiming at custom doors
    autoPairDoubleDoors = true, -- try to save the matching second panel for double doors
    doubleDoorRadius = 2.5, -- max distance between double-door panels
    outlineColor = { r = 255, g = 213, b = 64, a = 255 }, -- yellow outline shown while selecting doors
    prompt = {
        enabled = true, -- show lock icon/text near saved doors
        distance = 1.2, -- distance to press E on a door lock
        drawDistance = 8.0, -- distance to draw the door lock prompt/icon
        textureDict = "commonmenu", -- texture dictionary for lock icons
        lockedTexture = "shop_lock", -- icon shown when locked
        unlockedTexture = "shop_tick_icon", -- icon shown when unlocked
        soundSet = "HUD_FRONTEND_DEFAULT_SOUNDSET", -- frontend sound set for lock/unlock
        lockSound = "PIN_BUTTON", -- sound played after locking
        unlockSound = "PIN_BUTTON" -- sound played after unlocking
    },
    scanFallback = true, -- allow broad object fallback when model whitelist misses custom MLO doors
    maxScanDoors = 20, -- safety limit for doors found during admin scans
    minDoorHeight = 1.4, -- scanner filter: minimum door-like height
    maxDoorDepth = 0.55, -- scanner filter: maximum door-like depth
    maxDoorWidth = 4.5, -- scanner filter: maximum door-like width
    maxGarageDoorDepth = 1.25, -- wider depth allowed for roll-up/sliding garage doors
    maxGarageDoorWidth = 12.0, -- wider width allowed for garage doors
    maxGarageDoorHeight = 7.0, -- tallest garage door object accepted by fallback scans
    garage = {
        distance = 6.0, -- horizontal press-E distance for garage door locks only
        drawDistance = 12.0, -- icon draw distance for garage door locks only
        lockRadius = 6.0, -- object lookup radius for saved garage door locks only
        physicalLock = true, -- physically hold locked garage doors so MLO auto-open triggers cannot bypass the lock
        openRatio = 1.0, -- DoorSystem open ratio used when an owner presses E to open an unlocked garage door
        openRate = 1.0, -- DoorSystem animation/automatic rate used while opening garage doors
        snapToClosed = true, -- return locked garage panels to their saved object position before freezing them
        iconOnly = true, -- hide the Press E text and only show the lock icon for garage doors
        detectLegacyByModel = true, -- lets older saved garage locks use garage distances when the model is garage-sized
        detectLegacyByRadius = true -- lets older saved garage locks with this larger radius use garage distances
    },
    doorModels = {
        'v_ilev_fh_frontdoor',
        'v_ilev_mm_door',
    ...
    houses = {} -- optional manual door config by house name/id
}

-- Map blips for exterior house locations.
-- For sale blips show unowned/listed houses. Purchased blips show houses already owned by a player.
Config.Blips = {
    enabled = true, -- master toggle for house map blips
    showForSale = true, -- show blips for unowned/listed houses
    showPurchased = true, -- show blips for owned houses
    sprite = 40, -- default GTA blip sprite
    scale = 0.62, -- default blip size
    forSale = {
        label = "House For Sale", -- map label for listed houses
        color = 2, -- green
        shortRange = true -- only show when nearby on the map/radar
    },
    purchased = {
        label = "Purchased House", -- map label for owned houses
        color = 3, -- blue
        shortRange = true -- only show when nearby on the map/radar
    }
}
...
-- Interior defaults.
-- defaultExit is used until an owner sets a custom exit from inside the house.
-- Owners can also save stash, wardrobe, and logout points from the house menu.
Config.Interior = {
    defaultExit = { x = 266.03, y = -1007.22, z = -101.01, h = 357.0 }, -- fallback shell exit
    enterOffset = { x = 0.0, y = 0.0, z = 0.0 }, -- offset applied when entering shell interiors
    instanceSpacing = { enabled = true, columns = 10, distance = 100.0 }, -- isolate each property shell in its own underground instance
    targetSize = { x = 1.0, y = 1.0, z = 1.5 }, -- third-eye zone size for stash/wardrobe/logout/exit
    promptDistance = 1.15, -- tighter radius so stash/wardrobe/exit prompts do not overlap as much
    drawDistance = 8.0 -- marker draw radius inside shell interiors
}

-- Sell-back settings for owners.
-- percent is how much of the property price is refunded.
Config.SellBack = {
    enabled = true, -- allow owners to sell their house back
    account = 'bank', -- account that receives the sell-back money
    percent = 70 -- percent of purchase price refunded
}

-- Defaults used when creating a new property.
-- Images are intentionally blank; the current owner HUD does not display images.
Config.Defaults = {
    price = 250000, -- default price for new properties
    tier = 1, -- default shell tier
    type = 'shell', -- default property type: shell, mlo, or ipl
    shell = 'shell_v16low', -- default shell model/name
    ipl = '', -- default IPL name for IPL properties
    mlo = '', -- default MLO note/name for MLO properties
    image = '' -- image support is disabled by default
}

-- Shell catalogue shown in the administrator property form.
-- `entry` is the gameplay exit/entry point relative to the shell origin.
-- Set x/y/z/h manually per shell when a custom shell uses a different doorway.
Config.Shells = {
    {
        model = 'shell_v16low',
...
-- Live shell thumbnails are captured locally; no webhook or upload is used.
Config.ShellPreview = {
    enabled = true,
...
-- Bounds used for placing property dots on the admin map panel.
Config.MapBounds = {
    minX = -4200.0, -- west edge of admin map projection
    maxX = 4500.0, -- east edge of admin map projection
    minY = -4300.0, -- south edge of admin map projection
    maxY = 8400.0 -- north edge of admin map projection
}
-- Furniture catalogue is embedded below so FXServer loads one shared configuration file.
-- Furniture catalogue adapted from qbcore-framework/qb-houses (GPL-3.0).
-- Placement, ownership and persistence are implemented natively by LC Housing V2.
Config = Config or {}
Config.Decoration = {
    enabled = true,
    command = 'decorate',
